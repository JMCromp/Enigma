...
function encrypt(type){
	...
	// check input
	let input=$(type+'input').value.toUpperCase().replace(' ','');
	if (!input.match(/^[A-Z]+$/i)){
		showError('The '+(type=='dec_' ? 'cipher' : 'plain')+'text must only contain the letters A-Z.');
		}
	
	// check plugboard
	let plug=[].slice.call($(type+'plugboard').getElementsByTagName('select'));
	let charmaps=[[],[]];
	...
	// check rotors
	let rotor=[].slice.call($(type+'rotors').getElementsByTagName('select'));
	let rotorSlot=[], rotorPos=[];
	...
	// get ring
	let ring=[].slice.call($(type+'rings').getElementsByTagName('select')).map(function(r){
...
let s=bttn.innerHTML.match(/show/i);
